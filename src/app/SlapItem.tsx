...
export const SlapItem = ({
  ...
  const mePlayingFocus =
    playingNow.item &&
    ((playingNow.item.type == "spotify" &&
      playingNow.item?.trackId == item.trackId) ||
      (playingNow.item.type == "youtube" &&
        playingNow.item?.videoId == item.videoId));
  ...
  return (
    <div
      ...
      className={
        "flex py-2 items-center border-b border-gray-100 px-4" +
    ...
    >
      ...
      {listType == "slapper" && (
        <>
          <div className=" w-80 -py-2 -mt-1  overflow-x-hidden pr-4 ">
            {item.clips.map((clip, i) => {
              ...
              return (
                <button
                  onClick={() =>
                    active ? defocus() : focusClip(clip.id, item.id, slapId)
                  }
                  key={i}
                  className={
                    "px-2 text-xs mt-1 mr-1 inline-flex items-center py-1 font-medium text-white rounded " +
                ...
                >
                  ...
                  <span className="opacity-50 ml-1">
                    {" "}
                    ({Math.round((clip.to - clip.from) / 1000)}s)
                  </span>
                </button>
              );
            })}
            {!readOnly && (
              <button
                ...
                key={i}
                className="px-2 text-xs mb-2 inline-flex items-center py-1 bg-gray-300 hover:bg-gray-400 font-medium text-white rounded"
              >
                +
              </button>
            )}
          ...
          <DropdownMenu
            options={[{ name: "Remove", onClick: () => onDelete() }]}
          />
        </>
      )}
    </div>
...
const DropdownMenu = ({ options }) => {
  ...
  return (
    <div className="relative">
      ...
      {contextMenu && (
        <div
          className="origin-top-right absolute right-8 top-0 mt-2 w-40 rounded-md shadow-xl bg-white ring-1 ring-black ring-opacity-5 divide-y divide-gray-100"
          role="menu"
